...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 14 17:53:17 2020
...
from networkx.algorithms.flow.utils import build_residual_network, CurrentEdge

# Modified networkx implementations:

def mod_aug_path(G, s, t, capacity, residual, two_phase, cutoff):
    """Implementation of the shortest augmenting path algorithm.
    """
    P = [] # Default: there are no augmenting paths
    if s not in G:
    ...
    def augment(path):
        ...
        for v in it:
            attr = R_succ[u][v]
            # comment abadithe: The line: {flow = min(flow, attr["capacity"] - attr["flow"])} seems to be 
            # introducing flow even to paths for which attr["capacity"] < attr["flow"] especially when capacity = 0
            if (attr["capacity"] < abs(attr["flow"])):
                flow = 0
            else:
...
def get_augmenting_path(
    ...
    True
    """
    R, P = mod_aug_path(G, s, t, capacity, residual, two_phase, cutoff)
